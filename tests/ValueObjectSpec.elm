...
import ValueObject exposing (ValidateResult(..), ValueObject(..), validate)


type PositiveIntItem
    = PositiveIntItem (ValueObject Int)


positiveIntItem : Int -> PositiveIntItem
positiveIntItem =
    ValueObject >> PositiveIntItem


unwrapPositiveIntItem : PositiveIntItem -> Int
unwrapPositiveIntItem (PositiveIntItem (ValueObject a)) =
    a


invalidPositiveIntItemErrorMessage : Int -> String
invalidPositiveIntItemErrorMessage =
    String.fromInt >> (++) "Value must be positive but: "
    ...
    let
        ...
        func x =
            if x > 0 then
                ...
                ValueObject.Err (invalidPositiveIntItemErrorMessage x)
    in
    validate func a


type RecordItem
    = RecordItem (ValueObject { x : Int, y : String })


recordItem : Int -> String -> RecordItem
recordItem x y =
    RecordItem (ValueObject { x = x, y = y })


unwrapRecordItem : RecordItem -> { x : Int, y : String }
unwrapRecordItem (RecordItem (ValueObject a)) =
    a


invalidRecordItemXErrorMessage : Int -> String
invalidRecordItemXErrorMessage =
    String.fromInt >> (++) "property `x` must be positive but: "


invalidRecordItemYErrorMessage : String
invalidRecordItemYErrorMessage =
    "property `y` must include some char but empty"


invalidRecordItemTotallyErrorMessage : Int -> String
invalidRecordItemTotallyErrorMessage =
    invalidRecordItemXErrorMessage >> (++) invalidRecordItemYErrorMessage
    ...
    let
        ...
        func r =
            case ( r.x < 0, String.length r.y == 0 ) of
                ( True, True ) ->
                    ValueObject.Err (invalidRecordItemTotallyErrorMessage r.x)

                ( False, True ) ->
                    ValueObject.Err invalidRecordItemYErrorMessage

                ( True, False ) ->
                    ValueObject.Err (invalidRecordItemXErrorMessage r.x)
...
suite =
    describe "validate" <|
        ...
        , test "works against a valid record " <|
            ...
            ++ List.map
                (\( item, errMsg ) ->
                    test ("works against an invalid record expected message:" ++ errMsg) <|
                        \_ ->
